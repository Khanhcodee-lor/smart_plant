...
class PiBleProvisioningClient extends ChangeNotifier {
  ...
  static const _discoverDelay  = Duration(milliseconds: 800);
  ...
  Future<void> scanDevices() async {
    ...
    try {
      ...
      await FlutterBluePlus.isScanning
          .where((v) => v == false)
          .first
          .timeout(_scanDuration + const Duration(seconds: 3), onTimeout: () => false);
  ...
  Future<void> connect(BluetoothDevice device) async {
    ...
    try {
      ...
      await device.connect(autoConnect: false, timeout: _connectTimeout, mtu: null);
      ...
      _connSub = device.connectionState.listen((cs) {
        if (cs == BluetoothConnectionState.disconnected && !_disposed) {
          ...
          _cleanup(disconnect: false).then((_) {
            _set(PiBleState.error, error: 'Mất kết nối BLE. Vui lòng kết nối lại.');
          });
        }
      });
      ...
      for (final s in services) {
        if (s.uuid.toString().toLowerCase() == _svcUuid) { svc = s; break; }
      }
      if (svc == null) throw Exception('Provisioning service không tìm thấy: $_svcUuid');
  ...
  Future<List<BluetoothService>> _discoverWithRetry(BluetoothDevice device) async {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      if (!device.isConnected) {
        throw Exception('Device bị ngắt kết nối trong quá trình discover services.');
      }
      try {
        _log('discoverServices attempt ${i + 1}…');
        return await device.discoverServices(subscribeToServicesChanged: false, timeout: 12);
      } catch (e) {
        lastErr = e;
        ...
        if (i < 2) await Future<void>.delayed(const Duration(milliseconds: 600));
      }
    }
  ...
  Future<PiResponse> sendCommand(
    ...
    _log('Command: $json (${bytes.length} bytes)');
    ...
    for (var attempt = 0; attempt < _maxRetries; attempt++) {
      ...
      try {
        // ✅ FIX: Dùng writeWithoutResponse = true
        // Spec cho phép "write hoặc writeWithoutResponse"
        // Write Without Response nhanh hơn và không gây drop kết nối
        // khi Pi đang bận xử lý (không cần GATT ack packet)
        final supportsWriteNoResp = _cmdChar!.properties.writeWithoutResponse;
        _log('Write mode: ${supportsWriteNoResp ? "withoutResponse" : "withResponse"}');

        await _cmdChar!.write(
          bytes,
          withoutResponse: supportsWriteNoResp,
          // allowLongWrite chỉ dùng với withoutResponse: false
          allowLongWrite: !supportsWriteNoResp,
        );
        _log('Command written (${bytes.length} bytes).');

        // Chờ notify từ status char với timeout đủ dài cho WiFi scan
        _log('Waiting for status notify (timeout: ${_commandTimeout.inSeconds}s)…');
        ...
        // Đợi thêm 100ms để Pi ghi xong result char
        ...
        if (parsed is! Map) throw FormatException('Không phải JSON object: $resStr');

        final response = PiResponse.fromJson(Map<String, dynamic>.from(parsed));
  ...
  Future<List<PiWifiNetwork>> scanWifi({int limit = 12}) async {
    final r = await sendCommand('scan_wifi', payload: {'limit': limit});
  ...
  Future<PiWifiStatus> connectWifi({required String ssid, required String password}) async {
    final r = await sendCommand('connect_wifi', payload: {'ssid': ssid, 'password': password});
    if (!r.ok) throw Exception(r.error ?? 'connect_wifi thất bại');
    return r.status ?? (throw Exception('Pi không trả về status sau connect_wifi'));
  }
  ...
  Future<void> _cleanup({required bool disconnect}) async {
    ...
    // Cancel subscriptions (order: st → scan → conn để tránh event mới sau cleanup)
    await _stSub?.cancel();   _stSub   = null;
    ...
    if (disconnect && _device != null) {
      try {
        ...
        _log('Disconnected.');
      } catch (e) { _log('Disconnect error (ignored): $e'); }
    }
    _device = null;
  }
